...
chunkS = 8;   
%% MPI_BCAST
fprintf('=== i) MPI_Bcast：基础(二项) 与 拓展(分段) ===\n');
...
%% MPI_SCATTER
fprintf('=== ii) MPI_Scatter：基础(分治树) 与 拓展(k-项树+分段) ===\n');
P = p.NumWorkers;
...
%% ========== iii) MPI_ALLGATHER ==========
fprintf('=== 3) MPI_Allgather：基础(递归倍增) 与 拓展(环形流水) ===\n');
...
%% ========== iv) MPI_ALLTOALL ==========
fprintf('=== 4) MPI_Alltoall：基础(轮转赛互配) 与 拓展(伙伴聚合) ===\n');
...
%% ========== v) MPI_REDUCE ==========
fprintf('=== 5) MPI_Reduce：基础(树规约) 与 拓展(k-项树+分段) ===\n');
